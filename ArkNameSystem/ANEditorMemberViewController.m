...
- (void)viewDidLoad {
  ...
  NSArray *rs = [[ArkNameDataContainer instance] fetchMember:self.memberName];
  if (rs.count && self.memberName) {
...
-(void)viewWillLayoutSubviews{
  ...
  self.rpInfo.frame = CGRectMake(self.view.frame.size.width / 2 - 100, 80, 200, 50);
  self.rpNameF.frame = CGRectMake(self.view.frame.size.width / 2 - 100, 130, 200, 50);
  self.deleteBtn.frame = CGRectMake(self.view.frame.size.width / 2 - 100, 200, 200, 50);
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
  ...
  if (buttonIndex == 1) {
    [[ArkNameDataContainer instance] removeMember:self.memberName];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
  ...
  [[ArkNameDataContainer instance] replaceMember:self.memberName newName:textField.text];
  [[ArkNameDataContainer instance] save];
  UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil
                                                  message:@"修改成功"
                                                 delegate:nil
