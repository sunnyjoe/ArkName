...
- (void)viewDidLoad {
  ...
  self.eventView.contentInset = UIEdgeInsetsMake(10, 0, 10, 0);
  [self.view addSubview:self.eventView];
  [self buildEventView];
  
  self.memberView = [UIScrollView new];
  self.memberView.contentInset = UIEdgeInsetsMake(10, 0, 10, 0);
...
-(void)viewWillLayoutSubviews{
  ...
  self.segmentedControl.frame = CGRectMake(13, 64, self.view.frame.size.width - 26, 40);
  ...
  self.memberView.frame = self.eventView.frame;
...
-(void)buildEventView{
  ...
  NSInteger btnWidht = 150;
  NSInteger btnHeight = 80;
