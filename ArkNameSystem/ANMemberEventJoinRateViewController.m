...
-(void)buildUI{
  ...
  for (NSString *member in sortedName) {
    UILabel *bgLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, yHeight, self.contentScrollView.frame.size.width - 20, 30)];
    ...
    textLabel.text = [NSString stringWithFormat:@"%@", member];
    [textLabel setFont:[UIFont fontWithName:@"HelveticaNeue" size:20]];
    
    UILabel *numberLabel = [[UILabel alloc] initWithFrame:CGRectMake(bgLabel.frame.size.width - 100, 0, 95, bgLabel.frame.size.height)];
    numberLabel.backgroundColor = [UIColor clearColor];
    [bgLabel addSubview:numberLabel];
    numberLabel.textAlignment = NSTextAlignmentRight;
    numberLabel.textColor = [UIColor blackColor];
    numberLabel.text = [NSString stringWithFormat:@"%.0f", time];
    [numberLabel setFont:[UIFont fontWithName:@"HelveticaNeue" size:20]];
    
    yHeight += bgLabel.frame.size.height + 10;
  }
